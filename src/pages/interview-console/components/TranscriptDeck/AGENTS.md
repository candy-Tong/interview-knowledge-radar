...
展示服务端已按 5 秒静音规则合并的面试官 turns 和其 Q1/Q2 拆题，并允许用户选择历史 turn 或具体问题。组件不参与 ASR fragment 合并，也不触发检索。
...
- 行号仍按原始时间顺序计算。
- 原文内层使用可聚焦的 `role="button"` 选择 turn；拆题使用原生 button，`aria-pressed` 与 `selectedQuestionId` 同步。
...
- 转写列表可独立纵向滚动，不能撑高页面。
- 点击原文调用 `onSelect(itemId)`，点击问题调用 `onSelect(itemId, questionId)`；对应知识切换由页面派生状态处理。
