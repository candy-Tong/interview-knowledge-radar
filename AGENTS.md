...
- 用户提出涉及本项目且需要调查、修复或实现的问题时，开始处理前先创建 GitHub Issue，记录问题背景、期望结果和验收条件。
- 实现过程中以该 Issue 作为交付记录；不要在问题尚未解决或验证尚未通过时提前关闭。
- 完成后先在 Issue 中回写解决方案、关键改动、验证结果和对应 commit，再关闭 Issue。
...
- 连续语音只有静音超过 5 秒才形成新的面试官行；说话过程中使用增量原文节流刷新同一行的知识，ASR 完成后立即用完整原文校准，不等待翻译或 5 秒分行。
...
- 实时协议：运行 `server/realtime/translation-proxy.test.ts`，确认两种模式选择正确上游、普通模式无翻译事件、增量原文可提前检索、过期结果不会覆盖新结果，并且相邻 ASR 片段只产生一行。
